...
const Toprated = () => {
  ...
  useEffect(() => {
    async function fetch() {
      await fetchdata(null, 'findTopRated', null);
    }
    fetch();
  }, []);
  ...
  const setScrollIndex = (scrollVal) => {
    ...
    if (result < 0)
      setScrolling(0);
    else if (result > listRef.current?.scrollWidth - listRef.current?.clientWidth + scrollVal)
      setScrolling(0);
    else
      setScrolling(result);
  };
  ...
  return (
    <div id="Toprated" className='flex flex-col lg:items-center mx-10'>
      <span className="bg-gradient-to-r from-color-lightblue to-color-violet text-transparent font-poppins text-5xl font-semibold bg-clip-text py-2">
        Our top rated vehicles by clients
      </span>
      ...
      <div className='flex flex-row items-center justify-center gap-8'>
        <button onClick={() => { setScrollIndex(-320) }} className='hover:bg-gray-100 transition-all duration-300 hover:scale-125 items-center rounded-3xl p-2 rotate-90 scale-150 left-12 z-20'>
          <img src={downarrow} />
        </button>
        ...
        <button onClick={() => { setScrollIndex(320) }} className='hover:bg-gray-100 transition-all duration-300 hover:scale-125 items-center rounded-3xl p-2 -rotate-90 scale-150 right-12 z-20'>
          <img src={downarrow} />
        </button>
      </div>
    </div>
